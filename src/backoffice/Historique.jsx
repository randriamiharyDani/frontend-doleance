  ...
  if (!isAuthorized) {
    return (
      <div className="min-h-screen flex items-center justify-center bg-slate-50 dark:bg-slate-900 p-4">
        <div className="bg-white dark:bg-slate-800 rounded-3xl shadow-xl max-w-md w-full p-8 text-center ring-1 ring-slate-100 dark:ring-slate-700">
    ...
    <div className="mx-auto space-y-6">
          ...
          <button
            ...
            className={`inline-flex items-center gap-2 px-4 py-2.5 rounded-xl text-sm font-semibold transition-all duration-200 ring-1 shadow-sm ${
              filter === key
                ...
                : 'bg-white dark:bg-slate-800 text-slate-600 dark:text-slate-300 ring-slate-200 dark:ring-slate-700 hover:bg-slate-50 dark:hover:bg-slate-700 hover:ring-slate-300'
            }`}
          >
      ...
      <form onSubmit={handleSearch} className="flex gap-2">
        <div className="relative flex-1">
          ...
          <input
            ...
            className="w-full pl-9 pr-3 py-2.5 bg-white dark:bg-slate-800 border-2 border-transparent rounded-xl text-sm outline-none focus:border-[#1E3A8A]/30 focus:bg-white dark:focus:bg-slate-800 transition-all ring-1 ring-slate-200 dark:ring-slate-700"
          />
        </div>
        <button
          type="submit"
      ...
      <div className="flex items-center justify-between">
        <p className="text-sm text-slate-500 dark:text-slate-400">
          <span className="font-bold text-[#0F172A] dark:text-white">{pagination.total}</span> doléance{pagination.total > 1 ? 's' : ''} trouvée{pagination.total > 1 ? 's' : ''}
        </p>
      </div>
      ...
      <div className="bg-white dark:bg-slate-800 rounded-2xl shadow-sm ring-1 ring-slate-100 dark:ring-slate-700 overflow-hidden">
        {loading && doleances.length === 0 ? (
          ...
          <div className="text-center py-16 px-4">
            <div className="w-14 h-14 bg-slate-50 dark:bg-slate-700 rounded-2xl flex items-center justify-center mx-auto mb-3">
              <InboxIcon className="w-7 h-7 text-slate-300 dark:text-slate-500" />
            </div>
              ...
              <table className="min-w-full text-xl">
                <thead>
                  <tr className="border-b border-slate-100 dark:border-slate-700">
                    ...
                    <th className="px-4 py-3 text-left text-[13px] font-bold text-slate-400 dark:text-slate-500 uppercase tracking-wide">Résolu le</th>
                  </tr>
                </thead>
                <tbody className="divide-y divide-slate-50 dark:divide-slate-700">
                  {doleances.map((d) => (
                    <tr key={d.id_doleance} className="hover:bg-slate-50/70 dark:hover:bg-slate-700/50 transition-colors">
                      ...
                      <td className="px-4 py-3.5 text-slate-700 dark:text-slate-200 max-w-[200px] truncate text-sm">
                        {d.titre}
                      </td>
                      ...
                      <td className="px-4 py-3.5 whitespace-nowrap text-slate-500 dark:text-slate-400 text-sm">
            ...
            <div className="sm:hidden divide-y divide-slate-50 dark:divide-slate-700">
              {doleances.map((d) => (
                <div key={d.id_doleance} className="p-4 space-y-2">
                  ...
                  <p className="text-sm font-semibold text-slate-800 dark:text-slate-100">{d.titre}</p>
        ...
        <div className="flex items-center justify-between">
          ...
          <div className="flex gap-2">
            <button
              ...
              className="inline-flex items-center gap-1 px-3 py-2 bg-white dark:bg-slate-800 text-slate-600 dark:text-slate-300 rounded-xl text-sm font-semibold ring-1 ring-slate-200 dark:ring-slate-700 hover:bg-slate-50 dark:hover:bg-slate-700 disabled:opacity-40 disabled:cursor-not-allowed transition-colors"
            >
