...
function SatisfactionCard({ data }) {
  ...
  return (
    <div className="bg-white dark:bg-slate-800 rounded-xl shadow-sm border border-gray-100 dark:border-slate-700 p-4 md:p-6">
      <h2 className="text-lg font-semibold text-gray-700 dark:text-gray-200 mb-4">Satisfaction citoyenne</h2>
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        <div className="bg-emerald-50 rounded-xl p-4 text-center border border-emerald-100">
          <div className="text-2xl font-bold text-emerald-600">{data.taux_satisfaction || 0}%</div>
          <p className="text-xs text-gray-500 dark:text-gray-400 mt-1 font-medium">Taux satisfaction</p>
        ...
        <div className="bg-blue-50 rounded-xl p-4 text-center border border-blue-100">
          ...
          <p className="text-xs text-gray-500 dark:text-gray-400 mt-1 font-medium">Note moyenne</p>
          <div className="flex justify-center mt-2">
            {[...Array(5)].map((_, i) => (
              <StarIcon key={i} className={`h-4 w-4 ${i < Math.round(data.note_moyenne || 0) ? 'text-yellow-400 fill-current' : 'text-gray-300 dark:text-gray-600'}`} />
            ))}
          </div>
        </div>
        <div className="bg-purple-50 rounded-xl p-4 text-center border border-purple-100">
          <div className="text-2xl font-bold text-purple-600">{data.total_avis || 0}</div>
          <p className="text-xs text-gray-500 dark:text-gray-400 mt-1 font-medium">Avis reçus</p>
